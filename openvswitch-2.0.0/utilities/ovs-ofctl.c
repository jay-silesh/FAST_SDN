...
//Jay's declarations
static char *temp_char[3];
//static char *temp_char2;
static bool add_flow_flag=true;
int temp_i;
static void ofctl_flow_mod(int argc, char *argv[], uint16_t command);

//Number of hosts created so that we can split the 255 tables...
//static int number_of_host = 6;
//static int get_state_table_one(int temp_scr, int temp_dest);
static char * toArray(int number);
static char * ins_resubmit(int src,int dest, int table_no);
...
//Jay ...START
static char*
ins_resubmit(int src,int dest, int table_no)
{
        //Jay
        //use temp_char after this..
        char *temp_char2 = calloc(100, sizeof(char));

        //table=0 idle_timeout=10000 in_port=1 actions=resubmit(,1)";
        strcat( temp_char2 , " table=");
        if(table_no != 0)
           strcat( temp_char2 , toArray(table_no));
        else
           strcat( temp_char2, "0");
        strcat( temp_char2 , " idle_timeout=10000 in_port=");

        if( src != 0)
           strcat( temp_char2 , toArray(src));
        else
           strcat( temp_char2, "0");
        strcat( temp_char2 , " actions=resubmit(,");

        if(dest != 0)
           strcat( temp_char2 , toArray(dest));
        else
           strcat( temp_char2, "0");
        strcat( temp_char2 , ")");
...
static char * toArray(int number)
{
        ...
        {
                numberArray[i] = (char)(('0')+ (number % 10));
        }
        return numberArray;
}
...
static void
ins_state_table(int argc, char *argv[])
{
        //if you are using the function ins_resubmit then use the varibale temp_char..

        printf("\nJay : Will be inserting the custom tables...!\n");
        if( add_flow_flag )
        {
                //      temp_char2 = calloc(100, sizeof(char));
                temp_char[0] = "add-flow";
                temp_char[1] = "s1";
                add_flow_flag = false;
        }
        temp_char[2] = ins_resubmit(1, 2, 10);
        printf("\nSending the info with: %s\n",temp_char[2]);
        ofctl_flow_mod(3, temp_char, OFPFC_ADD);


        temp_char[2] = ins_resubmit(3, 4, 20);
        printf("\nSending the info with: %s\n",temp_char[2]);
        ofctl_flow_mod(3, temp_char, OFPFC_ADD);


        temp_char[2] = ins_resubmit(5, 6, 30);
        printf("\nSending the info with: %s\n",temp_char[2]);
        ofctl_flow_mod(3, temp_char, OFPFC_ADD);
...
static const struct command all_commands[] = {
    ...
    //Jay
    { "ins-state-tables", 1, 2, ins_state_table },
